#!/bin/sh
# Luke's Auto Rice Boostrapping Script (CARBS)
...
# DEFAULTS:
[ -z "$dotfilesrepo" ] && dotfilesrepo="git@gitlab.com:rholak/dotfiles.git" && repobranch="master"
[ -z "$progsfile" ] && progsfile="https://raw.githubusercontent.com/codefriendly/CARBS/master/archi3/progs.csv"
...
finalize(){ \
	...
	#echo "exec_always --no-startup-id notify-send -i ~/.local/share/larbs/larbs.png 'Welcome to CARBS:' 'Press Super+F1 for the manual.' -t 10000"  >> "/home/$name/.config/i3/config"
...
dialog --title "CARBS Installation" --infobox "Installing \`base-devel\` and \`git\` for installing other software." 5 70
...
# Install the CARBS Firefox profile in ~/.mozilla/firefox/
